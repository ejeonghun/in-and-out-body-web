...
class OrgTests(TestCase):
    def setUp(self):
        ...
        # 키오스크 회원가입
        response = self.client.post('/api/signup-kiosk/',
                                    {'session_key': self.session_key, 'phone_number': '01058581212',
                                     'password': '12345', 'auth_code': "0"}, format='json')
        ...
        # 키오스크 로그인
        response = self.client.post('/api/login-kiosk-id/',
                                    {'session_key': self.session_key, 'phone_number': '01058581212',
...
class GaitResultTests(TestCase):
    ...
    def test_get_gait_result_success(self):
        ...
        response = self.kiosk_client.post(base_url + 'api/analysis/count/',
                                          {"session_key": self.session_key, "type": 3}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class BodyResultTests(TestCase):
    ...
    def test_get_body_result_success(self):
        ...
        response2 = self.kiosk_client.post(base_url + 'api/analysis/count/',
                                           {"session_key": self.session_key, "type": 4}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
